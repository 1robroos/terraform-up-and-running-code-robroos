...
# get access to stage mysql database metadata:
...
data "terraform_remote_state" "global" {
  ...
  config = {
    ...
    key    = var.global_remote_state_key
    region = "us-east-2"
  }
}
resource "aws_launch_configuration" "example" {
  ...
  key_name             = data.terraform_remote_state.global.outputs.key
